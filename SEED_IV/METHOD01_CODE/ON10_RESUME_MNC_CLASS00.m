...
for File_path = 1%:3
    ...
    PARAMETER_MAIN_name = append(path_name,'\PARAMETER_MAIN_M_1.5_N_1.5_C_10_CLASS00.mat');
    PARAMETER_FUNCTION_name = append(path_name,'\PARAMETER_FUCNTION_M_1.5_N_1.5_C_10_CLASS00.mat');
    ...
    for RESUM_ROW_m = ROW_m:size(PARAMETER_m,1)
        for RESUM_ROW_n = ROW_n:size(PARAMETER_n,1)
            for RESUM_ROW_c00 = ROW_c00:size(PARAMETER_c,1)
                c00 = PARAMETER_c(RESUM_ROW_c00);
                ...
                create_PARAMETER_MAIN = append(create_path,'\PARAMETER_MAIN_M_',num2str(PARAMETER_m(RESUM_ROW_m)),'_N_',num2str(PARAMETER_n(RESUM_ROW_n)),'_C_',num2str(PARAMETER_c(RESUM_ROW_c00)),'_CLASS00.mat');
                PARAMETER_MAIN = {RESUM_ROW_m,RESUM_ROW_n,RESUM_ROW_c00};
                ...
                if PARAMETER_i == 0
                    if use_init_V
                        ...
                        V = F03_sgPFCMed_InitV (X,c00); % Initial cluster centers
                    end
                end
                ...
                create_PARAMETER_FUCNTION = append(create_path,'\PARAMETER_FUCNTION_M_',num2str(options(1)),'_N_',num2str(options(2)),'_C_',num2str(PARAMETER_c(RESUM_ROW_c00)),'_CLASS00.mat');
                
                E_old = 0;
                E_COUNT = 0;
                
                for i = PARAMETER_i+1:max_iter
                    [V, U, T, E(i)] = F04_sgPFCMed_Step(X, V, c00, m, eta,a,b,G);
                    ...
                    PARAMETER_FUCNTION = {i,V,U,T,E,X,c00,E_old,E_COUNT};
                ...
                GRID_SEARCH_MNC_CLASS00{1,1} = V;
                GRID_SEARCH_MNC_CLASS00{2,1} = U;
                GRID_SEARCH_MNC_CLASS00{3,1} = T;
                GRID_SEARCH_MNC_CLASS00{4,1} = E;
                
                create_GRID_SEARCH_MNC_CLASS00 = append(create_path,'\PROTOTYPE_M_',num2str(PARAMETER_m(RESUM_ROW_m)),'_N_',num2str(PARAMETER_n(RESUM_ROW_n)),'_C_',num2str(PARAMETER_c(RESUM_ROW_c00)),'_CLASS00.mat');
