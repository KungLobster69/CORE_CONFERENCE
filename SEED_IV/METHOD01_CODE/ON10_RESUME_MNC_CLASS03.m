...
for File_path = 1%:3
    path_name = append('D:\KUNG_LOBSTER69\RESULT\SEED_IV\METHOD01\09.GRID_SEARCH\GRID_SEARCH_MNC\',num2str(File_path));
    PARAMETER_MAIN_name = append(path_name,'\PARAMETER_MAIN_M_1.5_N_1.5_C_15_CLASS03.mat');
    PARAMETER_FUNCTION_name = append(path_name,'\PARAMETER_FUCNTION_M_1.5_N_1.5_C_15_CLASS03.mat');
    ...
    for RESUM_ROW_m = ROW_m:size(PARAMETER_m,1)
        for RESUM_ROW_n = ROW_n:size(PARAMETER_n,1)
            for RESUM_ROW_c03 = ROW_c03:size(PARAMETER_c,1)
                c03 = PARAMETER_c(RESUM_ROW_c03);
                ...
                create_PARAMETER_MAIN = append(create_path,'\PARAMETER_MAIN_M_',num2str(PARAMETER_m(RESUM_ROW_m)),'_N_',num2str(PARAMETER_n(RESUM_ROW_n)),'_C_',num2str(PARAMETER_c(RESUM_ROW_c03)),'_CLASS03.mat');
                PARAMETER_MAIN = {RESUM_ROW_m,RESUM_ROW_n,RESUM_ROW_c03};
                save(create_PARAMETER_MAIN,'PARAMETER_MAIN','-v7.3')
                
                GRID_SEARCH_MNC_CLASS03 = {};
                ...
                if PARAMETER_i == 0
                    if use_init_V
                        ...
                        V = F03_sgPFCMed_InitV (X,c03); % Initial cluster centers
                    end
                end
                ...
                create_PARAMETER_FUCNTION = append(create_path,'\PARAMETER_FUCNTION_M_',num2str(options(1)),'_N_',num2str(options(2)),'_C_',num2str(PARAMETER_c(RESUM_ROW_c03)),'_CLASS03.mat');
                
                E_old = 0;
                E_COUNT = 0;
                
                for i = PARAMETER_i+1:max_iter
                    [V, U, T, E(i)] = F04_sgPFCMed_Step(X, V, c03, m, eta,a,b,G);
                    ...
                    PARAMETER_FUCNTION = {i,V,U,T,E,X,c03,E_old,E_COUNT};
                ...
                GRID_SEARCH_MNC_CLASS03{1,1} = V;
                GRID_SEARCH_MNC_CLASS03{2,1} = U;
                GRID_SEARCH_MNC_CLASS03{3,1} = T;
                GRID_SEARCH_MNC_CLASS03{4,1} = E;
                
                create_GRID_SEARCH_MNC_CLASS03 = append(create_path,'\PROTOTYPE_M_',num2str(PARAMETER_m(RESUM_ROW_m)),'_N_',num2str(PARAMETER_n(RESUM_ROW_n)),'_C_',num2str(PARAMETER_c(RESUM_ROW_c03)),'_CLASS03.mat');
                save(create_GRID_SEARCH_MNC_CLASS03,'GRID_SEARCH_MNC_CLASS03','-v7.3')
